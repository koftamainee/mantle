...
namespace mantle {
    ...
    MeshHandle
    GPUResourceManager::upload_mesh(std::span<const Vertex> vertices,
        ...
        auto staging_vb = m_impl->vulkan_resources.create_buffer(
            vertex_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
            VMA_MEMORY_USAGE_CPU_ONLY, &vertex_mapped);
        auto staging_ib = m_impl->vulkan_resources.create_buffer(
            index_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
            VMA_MEMORY_USAGE_CPU_ONLY, &index_mapped);
        ...
        m_impl->device.copy_buffer(
            ...
            m_impl->vulkan_resources.get_buffer(vb), transfer_queue,
            vertex_size);
        m_impl->device.copy_buffer(
            m_impl->vulkan_resources.get_buffer(staging_ib),
            m_impl->vulkan_resources.get_buffer(ib), transfer_queue,
            index_size);
    ...
    bool GPUResourceManager::is_valid(MeshHandle handle) const {
        check(m_is_initialized);
        check(m_impl);
        if (handle.id >= m_impl->meshes.size()) {
            return false;
        }
