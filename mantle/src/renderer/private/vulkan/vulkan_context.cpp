...
namespace {
    ...
    VKAPI_ATTR VkBool32 VKAPI_CALL debug_callback(
        ...
        constexpr auto type_to_str = [](const VkDebugUtilsMessageTypeFlagsEXT t) -> const char * {
        ...
        auto msg = fmt::format("[VVL] [{}] {}", type_to_str(type), p_callback_data->pMessage);
...
namespace mantle {
    ...
    void VulkanContext::create_instance() {
        ...
        instance_create_info.enabledLayerCount = static_cast<u32>(ms_validation_layers.size());
        instance_create_info.ppEnabledLayerNames = ms_validation_layers.data();
#endif

        MANTLE_VK_VERIFY(vkCreateInstance(&instance_create_info, m_alloc_callbacks, &m_instance));
    ...
    void VulkanContext::create_debug_messenger_ext() {
        ...
        MANTLE_VK_VERIFY(vk_create_debug_utils_messenger(m_instance, &create_info,
                                                         m_alloc_callbacks, &m_debug_messenger));
    ...
    void VulkanContext::destroy_debug_messenger_ext() {
        if (m_debug_messenger != VK_NULL_HANDLE) {
            ...
            if (vk_destroy_debug_utils_messenger != nullptr) {
                vk_destroy_debug_utils_messenger(m_instance, m_debug_messenger, m_alloc_callbacks);
            }
    ...
    void VulkanContext::create_surface(SDL_Window *window) {
        ...
        MANTLE_FATAL(!SDL_Vulkan_CreateSurface(window, m_instance, m_alloc_callbacks, &m_surface),
    ...
    VulkanContext::get_required_instance_extensions(std::pmr::memory_resource &resource) {
        Uint32             sdl_extensions_count = 0;
        const char *const *sdl_extensions = SDL_Vulkan_GetInstanceExtensions(&sdl_extensions_count);
        MANTLE_FATAL(sdl_extensions == nullptr, "Failed to get SDL Vulkan extensions");
        ...
        MANTLE_VK_VERIFY(vkEnumerateInstanceExtensionProperties(nullptr, &vk_extensions_count,
                                                                vk_extensions.data()));
    ...
    void VulkanContext::check_validation_layers(std::pmr::memory_resource &memory) {
        u32 vk_layer_properties_count = 0;
        MANTLE_VK_VERIFY(vkEnumerateInstanceLayerProperties(&vk_layer_properties_count, nullptr));

        std::pmr::vector<VkLayerProperties> vk_layers(&memory);
        vk_layers.resize(vk_layer_properties_count);
        MANTLE_VK_VERIFY(
            vkEnumerateInstanceLayerProperties(&vk_layer_properties_count, vk_layers.data()));
