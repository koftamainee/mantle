...
namespace mantle {
    class VulkanDevice final {
      public:
        ...
        SwapchainSupportDetails get_swapchain_support_details(VkSurfaceKHR               surface,
        ...
        VkResult copy_buffer(VkBuffer src, VkBuffer dst, VkQueue queue, VkDeviceSize size,
                             VkDeviceSize src_offset = 0, VkDeviceSize dst_offset = 0) const;

        VkCommandPool
        create_command_pool(u32                      queue_family_index,
                            VkCommandPoolCreateFlags create_flags =
                                VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT) const;

        VkCommandBuffer create_command_buffer(VkCommandBufferLevel level, VkCommandPool pool,
        ...
        void end_single_time_commands(VkCommandBuffer command_buffer, VkQueue queue) const;
      ...
      private:
        ...
        bool is_physical_device_supports_required_extensions(VkPhysicalDevice physical_device);
