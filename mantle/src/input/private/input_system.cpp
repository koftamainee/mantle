...
InputSystem::ActiveDevice InputSystem::pick_active_device(const Window &window) const {
    ...
    switch (m_preference) {
        ...
        default:
            if (window.is_controller_active_this_frame())  return ActiveDevice::Controller;
            if (window.is_keyboard_active_this_frame())     return ActiveDevice::KeyboardMouse;
            if (window.is_mouse_active_this_frame())        return ActiveDevice::KeyboardMouse;
...
InputSystem::ActiveDevice InputSystem::get_active_device() const {
    return m_active_device;
}
...
f32 InputSystem::resolve_mouse_axis_raw(const ActionBinding &b) const {
    ...
    switch (*b.kbm.mouse_axis) {
        case MouseAxis::MoveX:  raw = window->get_mouse_delta().x * m_mouse_norm_x; break;
        case MouseAxis::MoveY:  raw = window->get_mouse_delta().y * m_mouse_norm_y; break;
        case MouseAxis::WheelX: raw = window->get_mouse_wheel().x;                  break;
        case MouseAxis::WheelY: raw = window->get_mouse_wheel().y;                  break;
    }
...
bool InputSystem::resolve_pressed(Action action) const {
    ...
    if (m_active_device == ActiveDevice::KeyboardMouse) {
        ...
        if (b.kbm.key.has_value() && window->is_key_pressed(*b.kbm.key))
            return true;
        if (b.kbm.mouse_button.has_value() && window->is_mouse_button_pressed(*b.kbm.mouse_button))
            return true;
    ...
    if (b.controller.button.has_value() && window->is_controller_button_pressed(*b.controller.button))
        return true;
...
bool InputSystem::resolve_just_pressed(Action action) const {
    ...
    if (m_active_device == ActiveDevice::KeyboardMouse) {
        ...
        if (b.kbm.key.has_value() && window->is_key_just_pressed(*b.kbm.key))
            return true;
        if (b.kbm.mouse_button.has_value() && window->is_mouse_button_just_pressed(*b.kbm.mouse_button))
            return true;
        if (b.kbm.axis_negative.has_value() && window->is_key_just_pressed(*b.kbm.axis_negative))
            return true;
        if (b.kbm.axis_positive.has_value() && window->is_key_just_pressed(*b.kbm.axis_positive))
            return true;
        return false;
    }
...
bool InputSystem::resolve_just_released(Action action) const {
    ...
    if (m_active_device == ActiveDevice::KeyboardMouse) {
        ...
        if (b.kbm.key.has_value() && window->is_key_just_released(*b.kbm.key))
            return true;
        if (b.kbm.mouse_button.has_value() && window->is_mouse_button_just_released(*b.kbm.mouse_button))
            return true;
        if (b.kbm.axis_negative.has_value() && window->is_key_just_released(*b.kbm.axis_negative))
            return true;
        if (b.kbm.axis_positive.has_value() && window->is_key_just_released(*b.kbm.axis_positive))
            return true;
        return false;
    }
...
f32 InputSystem::resolve_axis(Action action) const {
    ...
    if (m_active_device == ActiveDevice::KeyboardMouse) {
        ...
        if (b.kbm.axis_negative.has_value() || b.kbm.axis_positive.has_value()) {
            const f32 neg = b.kbm.axis_negative.has_value() && window->is_key_pressed(*b.kbm.axis_negative) ? -1.0f : 0.0f;
            const f32 pos = b.kbm.axis_positive.has_value() && window->is_key_pressed(*b.kbm.axis_positive) ?  1.0f : 0.0f;
            return neg + pos;
        }
