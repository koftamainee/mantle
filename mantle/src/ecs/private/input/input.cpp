...
namespace mantle {
    ...
    void bootstrap_input(const flecs::world &world, Window &window) {
        ...
        world.system<>("Input")
            ...
            .run([&window](const flecs::iter &it) {
                ...
                f32 rx = apply_deadzone(
                    window.get_controller_axis(Window::ControllerAxis::RightX),
                    stick_deadzone);
                f32 ry = apply_deadzone(
                    window.get_controller_axis(Window::ControllerAxis::RightY),
                    stick_deadzone);
                if (rx != 0.0f || ry != 0.0f) {
                ...
                f32 lx = apply_deadzone(
                    window.get_controller_axis(Window::ControllerAxis::LeftX),
                    stick_deadzone);
                f32 ly = apply_deadzone(
                    window.get_controller_axis(Window::ControllerAxis::LeftY),
                    stick_deadzone);
                if (lx != 0.0f || ly != 0.0f) {
                ...
                f32 lt =
                    apply_deadzone(window.get_controller_axis(
                ...
                if (!controller_active) {
                    ...
                    player_input.sprint = player_input.sprint ||
                        window.is_key_pressed(Window::Key::LControl);
                }
            });
    }
} // namespace mantle
