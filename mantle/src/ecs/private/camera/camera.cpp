    ...
    void bootstrap_camera(const flecs::world &world, f32 aspect) {
        ...
        world.system<>("Camera")
            ...
            .run([](const flecs::iter &it) {
                ...
                camera.pitch =
                    glm::clamp(camera.pitch + input_state.look_dy, -89.f, 89.f);

                glm::vec3 front;
                front.x = std::sin(glm::radians(camera.yaw)) *
                    std::cos(glm::radians(camera.pitch));
                front.y = std::sin(glm::radians(camera.pitch));
                front.z = -std::cos(glm::radians(camera.yaw)) *
                    std::cos(glm::radians(camera.pitch));
                camera.front = glm::normalize(front);
                camera.right = glm::normalize(
                    glm::cross(camera.front, glm::vec3(0.f, 1.f, 0.f)));

                f32 speed = camera_base_speed *
                    (input_state.sprint ? camera_sprint_multiplier : 1.0f) *
                    it.delta_time();

                camera.position +=
                    camera.front * input_state.move_forward * speed;
                camera.position +=
                    camera.right * input_state.move_strafe * speed;
                camera.position +=
                    glm::vec3(0.0f, 1.0f, 0.0f) * input_state.move_up * speed;
