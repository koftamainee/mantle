...
#define DO_CHECK 1
    ...
    debug_assert_failed(const char *expr, std::source_location loc,
                        const char *fmt, Args &&...args) {
        spdlog::critical(
            "Assertion failed: ({})\n  File: {}\n  Line: {}\n  Function: {}",
            expr, project_filename(loc.file_name()),
    ...
    template <typename... Args>
    [[noreturn]] inline void
    debug_fatal_failed(u32 error_code, std::source_location loc,
                       const char *fmt, Args &&...args) {
#if DO_CHECK
        spdlog::critical(
            "Fatal error\n  File: {}\n  Line: {}\n  Function: {}\n  Code: E{:08X}",
            project_filename(loc.file_name()),
            loc.line(), loc.function_name(), error_code);
        spdlog::critical(fmt, std::forward<Args>(args)...);
#else
    ...
    template <typename... Args>
    inline void
    debug_ensure_failed(const char *expr, std::source_location loc,
                        const char *fmt, Args &&...args) {
        spdlog::warn(
            "Ensure failed: ({})\n  File: {}\n  Line: {}\n  Function: {}",
            expr, project_filename(loc.file_name()),
...
#define MANTLE_FATAL(expr, fmt, ...)                                    \
    do {                                                                \
        if (MANTLE_UNLIKELY(expr)) {                                    \
            ...
            constexpr ::mantle::u32 _code = ::mantle::fnv1a(                      \
                ::mantle::project_filename(_loc.file_name()),            \
                _loc.line());                                           \
            ::mantle::debug_fatal_failed(                               \
                _code, _loc, fmt, ##__VA_ARGS__);                       \
        }                                                               \
    } while (0)
